...
int main(int argc, char** argv) {
    ...
    if (argc != 3){
	cerr << "Usage: aia6 vectLen numberOfComponents" << endl;
	return -1;
    }
    
    // some parameters
    int vectLen = atoi(argv[1]);			// dimensionality of the generated feature vectors (number of principal components)
    int numberOfComponents = atoi(argv[2]); 	// number of components (clusters) to use by model
    cout << vectLen << endl;
    cout << numberOfComponents << endl;
/*
    int vectLen = 1;			// dimensionality of the generated feature vectors (number of principal components)
    int numberOfComponents = 1; 	// number of components (clusters) to use by model
*/
