...
int main(int argc, char** argv) {
	...
	if (argc > 2) {
		numberOfComponents = atoi(argv[2]);
	}
...
void plotGMM(vector<struct comp*>& model, Mat& features) {
	...
	for (int i=0; i<model.size(); i++) {
		...
		// draw components
		circle(plot,  Point(
				(model.at(i)->mean.at<float>(1,0)-min_y)*scale,
				(model.at(i)->mean.at<float>(0,0)-min_x)*scale ),
				3, Scalar(0,0,255), 2);
		ellipse(plot, Point(
				(model.at(i)->mean.at<float>(1,0)-min_y)*scale,
				(model.at(i)->mean.at<float>(0,0)-min_x)*scale ),
				Size(EVal.at<float>(1)*scale*3,
				EVal.at<float>(0)*scale*3),
				rAng*180/CV_PI, 0, 360, Scalar(0,255,0), 1);
	}
